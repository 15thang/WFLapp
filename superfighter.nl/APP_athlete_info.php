...
<form name="form1" method="post" action="" enctype="multipart/form-data">
   <div id="titles" style="float: right">
    <form>
        <label for="ath_title">Add title:</label><br>
        <input type="text" id="ath_title" name="ath_title"><br><br>
        <button type="submit" class="btn" name="add_title">Add title</button>
    </form>
         <table>
           <tr>
               <th>Athlete titles</th>
           </tr>
</form>';
$query = "SELECT * FROM `athletetitles` WHERE athlete_id = '$athlete_id' ORDER BY `athletetitles`.`title_id` DESC";
$results = mysqli_query($db, $query);
while ($row = $results->fetch_assoc()) {
    echo '
           <tr>
    ...
    //Voeg een nieuwe titel aan een atleet toe.
    if (isset($_POST['add_title'])) {
        $title = ($_POST['ath_title']);

        header("Location: http://superfighter.nl/APP_add_title.php?athlete_id=".$athlete_id.'&title='.$title);
        ob_flush();
    }
echo '</table></div>
   </section>
