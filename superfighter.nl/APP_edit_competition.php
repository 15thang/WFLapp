...
<div id="field2">
    <form name="form1" method="post" action="" enctype="multipart/form-data">
        ...
        <?php
        $query = "SELECT competition_name FROM `competition` WHERE competition_id = '$comp_id'";
        $results = mysqli_query($db, $query);
        while ($row = $results->fetch_assoc()) {
            echo '<input type="text" name="comp_name" value="'.$row['competition_name'].'"><br>';
        }
        ?>
        <br>
</div>
<div id="field2">
    <div class="container" style="margin-top:50px;">
        <input type="submit" value="Edit competition" name="edit_comp">
        ...
        <table id="table_format" class="table table-bordered">
            ...
            <?php
            $query = "SELECT * FROM `athletes` WHERE athlete_id NOT IN(SELECT athlete_id FROM `athletecompetition` WHERE competition_id = '$comp_id') ORDER BY athlete_lastname ASC";
            $results = mysqli_query($db, $query);
            while ($row = $results->fetch_assoc()) {
    ...
    if(isset($_POST['edit_comp'])){
        ...
        $query = "UPDATE `competition` SET competition_name = '$comp_name' WHERE competition_id = '$comp_id'";
        mysqli_query($db, $query);

        //get event_id (event_0 id)
        $query2 = "SELECT event_id FROM events WHERE event_name = 'event_0' AND competition = '$comp_id'";
        ...
        $query4 = "SELECT match_id, redcorner FROM `eventcompetition` WHERE event_id = '$event0' AND competition_id = '$comp_id' 
